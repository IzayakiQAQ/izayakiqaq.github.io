...
function renderHome() {
  ...
  return `<main class="academic-home">
    <section class="academic-hero" aria-labelledby="academic-hero-title">
      <div class="academic-hero__copy">
        ...
        <div class="academic-icon-actions" aria-label="Academic profiles and contact">
          ${iconAction('GitHub', author.github, 'fa-brands fa-github')}
          ${iconAction('Google Scholar', author.scholar, 'fa-solid fa-graduation-cap')}
          ${iconAction('Email', author.email, 'fa-solid fa-envelope', 'email')}
          ${iconAction('ORCID', author.orcid, 'fa-brands fa-orcid')}
          ${iconAction('CV', author.cv, 'fa-solid fa-file-lines')}
        </div>
      </div>
      <div class="academic-hero__portrait">
        ${hasAvatar ? `<img src="${escapeHtml(siteUrl(author.avatar))}" alt="Portrait of ${escapeHtml(author.name)}">` : '<div class="academic-avatar-placeholder" role="img" aria-label="Profile photo placeholder"><span>PROFILE</span><strong>PhD</strong><small>{{PROFILE_PHOTO}}</small></div>'}
      </div>
    </section>
    ...
    <section class="academic-section">
      ${sectionHeading('Focus', 'Research interests', 'Three modular directions ready to be replaced with your verified research.', 'research/')}
    ...
    <section class="academic-section academic-section--quiet">
      ${sectionHeading('Notebook', 'Recent blog posts', 'Notes on research, learning, technology, and daily life.', 'blog/')}
      ${recentPosts(getPosts(), 3)}
    </section>
  </main>`;
}

function detailList(title, values) {
  return `<div class="academic-research-detail"><h4>${escapeHtml(title)}</h4><ul>${asArray(values).map(value => `<li>${escapeHtml(value)}</li>`).join('')}</ul></div>`;
}

function renderResearch() {
  const research = asArray(dataStore().research);
  return `<div class="academic-page academic-research-page">
    <p class="academic-page-lead">Research directions are maintained in <code>source/_data/research.yml</code>. Every entry below is placeholder content.</p>
    <div class="academic-research-stack">${research.map((item, index) => `<section id="${escapeHtml(item.id || `research-area-${index + 1}`)}" class="academic-research-panel">
      ...
      <div class="academic-research-panel__details">${detailList('Research questions', item.questions)}${detailList('Methods', item.methods)}${detailList('Current projects', item.projects)}${detailList('Related publications', item.publications)}${detailList('Related code', item.code)}</div>
    </section>`).join('')}</div>
  </div>`;
}
...
function renderPublications() {
  ...
  return `<div class="academic-page academic-publications-page">
    <p class="academic-page-lead">Publications are listed in reverse order from a single YAML file. Replace every clearly marked placeholder with verified bibliographic data.</p>
    ...
    <div class="academic-publication-list academic-publication-list--full">${publications.map(publicationItem).join('')}</div>
  </div>`;
}

function renderProjects() {
  const projects = asArray(dataStore().projects);
  return `<div class="academic-page academic-projects-page"><p class="academic-page-lead">Projects are rendered from <code>source/_data/projects.yml</code>; empty links remain visibly disabled until real resources are supplied.</p>${projectCards(projects)}</div>`;
}
...
function renderAbout() {
  ...
  return `<div class="academic-page academic-about-page">
    ...
    <section><h2>Education</h2>${timeline(data.education)}</section>
    <section><h2>Research experience</h2>${timeline(data.experience)}</section>
    <div class="academic-about-columns"><section><h2>Academic service</h2>${simplePlaceholderList(data.service)}</section><section><h2>Awards</h2>${simplePlaceholderList(data.awards)}</section></div>
    <section><h2>Contact</h2><div class="academic-icon-actions">${iconAction('Email', author.email, 'fa-solid fa-envelope', 'email')}${iconAction('GitHub', author.github, 'fa-brands fa-github')}${iconAction('ORCID', author.orcid, 'fa-brands fa-orcid')}</div></section>
  </div>`;
}

function renderCv() {
  const author = profile();
  const configured = author.cv && !isToken(author.cv);
  return `<div class="academic-page academic-cv-page">
    <div class="academic-cv-card"><span class="academic-eyebrow">Curriculum Vitae</span><h2>${escapeHtml(author.name)}</h2><p>${configured ? 'Open or download the current CV using the buttons below.' : 'The CV has not been supplied yet. Add a verified PDF and update the centralized profile configuration.'}</p><div class="academic-actions">${actionLink('View CV', author.cv, 'fas fa-eye')}${actionLink('Download PDF', author.cv, 'fas fa-download')}</div>${configured ? '' : '<code>{{CV_URL}}</code><small>Recommended local path: /files/cv.pdf</small>'}</div>
    <aside class="academic-cv-help"><h3>How to publish the CV</h3><ol><li>Copy the verified PDF to <code>source/files/cv.pdf</code>.</li><li>Set <code>author.cv: /files/cv.pdf</code> in <code>source/_data/profile.yml</code>.</li><li>Rebuild the site.</li></ol></aside>
  </div>`;
}
...
hexo.extend.tag.register('academic_cv', renderCv);
